...
(function(){
  ...
  function css(){
    ...
    st.textContent=`
      ...
      .credHero{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:16px;align-items:start;padding:16px 18px;border:1px solid var(--line);border-radius:20px;background:linear-gradient(135deg,rgba(22,119,255,.13),rgba(255,255,255,.025)),var(--panel2,#151a28);box-shadow:var(--shadow)}
      ...
      .credMode button.active,.credBtn.primary{background:linear-gradient(135deg,rgba(22,119,255,.32),rgba(75,127,255,.22));border-color:rgba(22,119,255,.8)}
      ...
      .credPlan.active{border-color:rgba(22,119,255,.9);background:rgba(22,119,255,.13)}
      ...
      .riskHead .credBtn{background:linear-gradient(135deg,rgba(22,119,255,.36),rgba(75,127,255,.24));border-color:rgba(22,119,255,.75);box-shadow:0 10px 22px rgba(75,127,255,.12)}
      ...
      .riskRec{border:1px solid rgba(22,119,255,.38);background:rgba(22,119,255,.10);border-radius:15px;padding:12px;line-height:1.45}
...
Observação: documento interno de apoio à decisão, baseado nos dados registrados no Cronos. Não substitui consulta a órgãos de crédito nem decisão administrativa da clínica.`;
  ...
  function riskResultHTML(){
    ...
    if(!r){
      return `<div class="riskEmpty riskEmptyV39"><div><b>Score ainda não calculado</b><p style="margin:8px 0 0">Preencha os dados ou use a simulação acima para gerar uma leitura interna de 0 a 1000.</p><p class="riskMini" style="margin:10px 0 0">O resultado apoia a decisão da clínica e não substitui consulta externa de crédito.</p></div></div>`;
    }
    const p=Math.max(0,Math.min(100,r.score/10));
  ...
  function calcCard(){
    ...
    return `<div class="credCard credStepCard credCalcCardV39"><div class="credStepLabel">Etapa 2</div><h3>Cálculo financeiro</h3><div class="credMuted" style="margin-bottom:12px">Informe proposta, entrada, prazo e taxa. O Cronos calcula o saldo financiado e a prestação.</div><div class="credForm"><div><label>Valor total/proposta</label><input id="credTotal" value="${esc(S.inputs.total||'')}" oninput="CRONOS_CREDITO.rememberInputs('total')" placeholder="Ex: 2930,00"></div><div class="credEntryPctGroup"><label>Entrada opcional</label><div class="credEntryPctInline"><input id="credEntry" value="${esc(S.inputs.entry||'')}" oninput="CRONOS_CREDITO.rememberInputs('entry')" placeholder="Ex: 500,00"><input id="credEntryPct" value="${esc(cleanExampleInput(S.inputs.entryPct||''))}" onfocus="if(/^ex(?:emplo)?\s*:?\s*20\s*%?$/i.test(this.value.trim()))this.value=''" oninput="CRONOS_CREDITO.rememberInputs('entryPct')" title="Digite a porcentagem para o Cronos calcular a entrada" placeholder="Ex: 20%"></div></div><div class="credAutoField"><label>Valor financiado/saldo</label><input id="credPV" value="${esc(S.inputs.pv||'')}" readonly aria-readonly="true" tabindex="-1" placeholder="calculado automaticamente"></div><div><label>Nº de meses</label><input id="credN" value="${esc(S.inputs.n||'')}" oninput="CRONOS_CREDITO.rememberInputs()" placeholder="Ex: 12"></div><div><label>Taxa de juros mensal (%)</label><input id="credRate" value="${esc(S.inputs.rp||'')}" oninput="CRONOS_CREDITO.rememberInputs()" placeholder="Ex: 2,5"></div><div class="credPaymentField"><label>Valor da prestação</label><input id="credPMT" value="${esc(S.inputs.P||'')}" readonly aria-readonly="true" tabindex="-1" placeholder="calculado automaticamente"></div></div><div class="credActions credActionsV41"><button class="credBtn primary" onclick="CRONOS_CREDITO.calc()">Calcular prestação</button><button class="credBtn subtle" onclick="CRONOS_CREDITO.clear()">Limpar</button>${r?`<button class="credBtn ok" onclick="CRONOS_CREDITO.copy()">Copiar</button>${c?.phone?`<button class="credBtn wa" onclick="CRONOS_CREDITO.whats()">WhatsApp</button>`:''}<button class="credBtn subtle iconOnly printBtnV41" onclick="CRONOS_CREDITO.print()" title="Imprimir simulação" aria-label="Imprimir simulação"><svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path fill="currentColor" d="M6 9V3h12v6h1a3 3 0 0 1 3 3v5h-4v4H6v-4H2v-5a3 3 0 0 1 3-3h1Zm2-4v4h8V5H8Zm8 10H8v4h8v-4Zm2 0h2v-3a1 1 0 0 0-1-1H5a1 1 0 0 0-1 1v3h2v-2h12v2Zm-1-1.5a1 1 0 1 0 0-2 1 1 0 0 0 0 2Z"/></svg></button>${e?`<button class="credBtn subtle" onclick="CRONOS_CREDITO.saveHistoryRecord()">Salvar histórico</button>`:''}`:''}</div>${S.error?`<div class="credAlert" style="margin-top:12px">${esc(S.error)}</div>`:''}</div>`;
  }
  ...
  function render(){
    const v=ensureView();
    v.innerHTML=`<div class="credWrap credWrapV39"><div class="credHero credHeroV39"><div class="credHeroTitleV39"><div class="credHeroIconV39" aria-hidden="true">$</div><div><h2>Simulador de crédito / pagamento</h2><p>Calcule parcelas, teste entradas e use o score interno para apoiar decisões de pagamento.</p></div></div><button class="credBtn subtle credRefreshBtn" onclick="CRONOS_CREDITO.refresh(this)"><span class="credRefreshSpinner" aria-hidden="true"></span><span class="credRefreshText">Atualizar</span></button></div><div class="credGrid"><div class="credLeftStack" style="display:grid;gap:16px;align-content:start;align-self:start"><div class="credCard credStepCard"><div class="credStepLabel">Etapa 1</div><h3>Tipo de simulação</h3><div class="credMode"><button class="${S.mode==='free'?'active':''}" onclick="CRONOS_CREDITO.setMode('free')">Simulação livre</button><button class="${S.mode==='linked'?'active':''}" onclick="CRONOS_CREDITO.setMode('linked')">Vincular paciente</button></div><p class="credMuted" style="margin-bottom:0">Livre para cálculos rápidos. Vinculada para usar dados reais do paciente.</p></div>${linkedCard()}</div>${calcCard()}</div>${resultCard()}${riskAnalysisCard()}${historyCard()}</div>`;
  }
  async function boot(){

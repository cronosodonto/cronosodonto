  ...
  try{
    ...
    function injectFichaStyles(){
      ...
      style.textContent = `
        ...
        .odontoStatusLayer .toothLine{fill:#111827;stroke:none;pointer-events:none;transition:fill .16s ease,opacity .16s ease;shape-rendering:geometricPrecision}
        body:not(.light) .odontoStatusLayer .toothLine{fill:rgba(226,232,240,.92)}
...
window.CRONOS_PROC_UI = {
    ...
    function renderOdontogramSVG(entry, opts = {}){
      const selectedTeeth = Array.isArray(opts.selectedTeeth) ? opts.selectedTeeth.map(String) : [];
      ...
      const upper = [...TOOTH_ROWS.supDir, ...TOOTH_ROWS.supEsq].map(String);
      const lower = [...TOOTH_ROWS.infDir, ...TOOTH_ROWS.infEsq].map(String);
      ...
      const lineMarkup = teeth.map(tooth=>{
        const d = ODONTO_CRONOS_ORIGINAL_PATHS[tooth] || '';
      ...
      const hitMarkup = interactive ? teeth.map(tooth=>{
        const box = ODONTO_CRONOS_ORIGINAL_BOXES[tooth] || [0,0,0,0];
        const x = Number(box[0] || 0), y = Number(box[1] || 0), w = Math.max(0, Number(box[2] || 0) - x), h = Math.max(0, Number(box[3] || 0) - y);
        return `<g class="${toothClass(tooth)}" data-tooth="${tooth}" tabindex="0" role="button" aria-label="Dente ${tooth}" onclick="CRONOS_FICHA_UI.toggleTooth('${tooth}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();CRONOS_FICHA_UI.toggleTooth('${tooth}')}"><rect class="toothHit" x="${x}" y="${y}" width="${w}" height="${h}" rx="18" ry="18"></rect></g>`;
      }).join('') : '';
      ...
      const upperLabelY = -10;
      const lowerLabelY = 822;

      return `<svg class="odontoStatusLayer" viewBox="${ODONTO_CRONOS_ORIGINAL_VIEWBOX}" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" preserveAspectRatio="xMidYMid meet"><g transform="translate(0,800) scale(0.100000,-0.100000)">${lineMarkup}</g></svg><svg class="odontoLabelLayer" viewBox="${ODONTO_CRONOS_ORIGINAL_VIEWBOX}" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" preserveAspectRatio="xMidYMid meet">${labelMarkup(upper, upperLabelY)}${labelMarkup(lower, lowerLabelY)}</svg>${interactive ? `<svg class="odontoClickLayer" viewBox="${ODONTO_CRONOS_ORIGINAL_VIEWBOX}" xmlns="http://www.w3.org/2000/svg" aria-label="Camada interativa do odontograma" preserveAspectRatio="xMidYMid meet">${hitMarkup}</svg>` : ''}`;
    }
          ...
          <div class="odontoGrid">
            <div>
              <div class="odontoRefStage">
                ${renderOdontogramSVG(entry, { selectedTeeth, interactive: true })}
              </div>
...
.legend{display:flex;gap:8px;flex-wrap:wrap;margin-top:8px;font-size:12px;color:#555}.legend span{display:inline-flex;align-items:center;gap:6px;border:1px solid #ddd;padding:5px 9px;border-radius:999px}
        ...
        <div class="sheet">
          ...
          <div class="boxWrap" style="margin-top:16px">
            <div class="sectionTitle">Odontograma</div>
            <div class="odonto">${renderOdontogramSVG(entry, { selectedTeeth: [], interactive: false, getVisualState: getPrintToothVisualState })}</div>
